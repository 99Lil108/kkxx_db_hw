...
const selectLoginStatus = (idx) => {
  if (!idx == state.loginStatus) {
    ...
    let role_name = "patient",
      last_name = "patient";
    if (idx == 1) role_name = "doctor";
    else role_name = "admin";
    if (last == 1) last_name = "doctor";
    else last_name = "admin";
    doms[last].children[0].src = `/src/assets/login/${role_name}_white.png`;
    doms[last].children[0].src = `/src/assets/login/${role_name}_blue.png`;
    console.log(roles.value.children[idx].children[0]);
    console.log(roles.value.children[last]);
  }
};
</script>
<template>
  <div class="content">
    ...
    <div class="site">
      ...
      <div class="login">
        <div class="main">
          <p>
            <input class="in" type="tel" placeholder="手机号" />
          </p>
          <p class="password">
