...
onMounted(() => {
  console.log(roles.value.children);
  roles.value.children.forEach((dom, idx) => {
    if (idx == state.loginStatus)
      dom.children[0].src = `/src/assets/login/${mapper[idx]}_white.png`;
    else dom.children[0].src = `/src/assets/login/${mapper[idx]}_blue.png`;
  });
});
</script>
<template>
